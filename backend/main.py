...
# # --- App Setup ---
# sio = socketio.AsyncServer(async_mode="asgi", cors_allowed_origins=ALLOWED_ORIGINS)
# app = FastAPI(title="Salarite API", version="2.1.0")
...
# # --- Helper to emit from sync routes ---
# def emit_event(event: str, data=None):
#     """Schedule a socket emit from a sync route safely."""
#     try:
#         loop = asyncio.get_event_loop()
#         if loop.is_running():
#             loop.create_task(sio.emit(event, data or {}))
#         else:
#             loop.run_until_complete(sio.emit(event, data or {}))
#     except Exception as e:
#         print(f"emit_event error: {e}")
...
#     # 🔴 Real-time emit
#     emit_event("tasks_changed")
#     emit_event("activity_new")
...
#         cur.execute("SELECT * FROM interviews WHERE created_by=%s ORDER BY scheduled_at ASC", (user["sub"],))
#     else:
#         cur.execute("SELECT * FROM interviews WHERE lower(candidate_email)=%s ORDER BY scheduled_at ASC", (user["email"].lower(),))
#     rows = cur.fetchall()
#     cur.close(); conn.close()
#     return [dict(r) for r in rows]
...
sio = socketio.AsyncServer(
    ...
    # Render pe long-polling timeout badhao
    ping_timeout=60,
    ping_interval=25,
)
app = FastAPI(title="Salarite API", version="2.2.0")
...
# --- FIXED: emit_event — sync routes se async emit karna ---
# asyncio.get_event_loop() Python 3.10+ mein deprecated hai aur Render pe fail hota hai
# asyncio.get_running_loop() use karo — yeh always works in async context (ASGI)
def emit_event(event: str, data=None):
    """Safely emit socket event from a sync FastAPI route."""
    try:
        loop = asyncio.get_running_loop()
        # create_task schedules it on the running event loop — guaranteed to fire
        loop.create_task(sio.emit(event, data or {}))
    except RuntimeError:
        # Fallback: agar kisi reason se loop nahi mila (shouldn't happen in ASGI)
        print(f"[emit_event] No running loop for event: {event}")
...
def list_tasks(user: dict = Depends(get_current_user)):
    ...
    if user["role"] == "employer":
        ...
        # candidate — koi tasks nahi dikhte
        cur.close(); conn.close()
        return []
    rows = cur.fetchall()
    cur.close(); conn.close()
...
def list_interviews(user: dict = Depends(get_current_user)):
    ...
    if user["role"] == "hr":
        ...
        # candidate aur employer dono apni email se match karate hain
        cur.execute(
            "SELECT * FROM interviews WHERE lower(candidate_email)=%s ORDER BY scheduled_at ASC",
            (user["email"].lower(),)
        )
    rows = cur.fetchall()
    cur.close(); conn.close()
...
def create_interview(body: InterviewCreateRequest, user: dict = Depends(require_roles("hr"))):
    ...
    log_activity(
        "interview_scheduled",
        f'Interview scheduled with {candidate_name or candidate_email}',
        user["sub"]
    )
