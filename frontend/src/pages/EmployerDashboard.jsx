...
export default function EmployerDashboard() {
  ...
  const [tasks, setTasks] = useState([]);
  const [activities, setActivities] = useState([]);
  const [form, setForm] = useState({ title: "", description: "", priority: "medium", assigned_hr_email: "" });
  const [loading, setLoading] = useState(true);
  const [creating, setCreating] = useState(false);
  ...
  const load = useCallback(async () => {
    try {
      const [t, a] = await Promise.all([api.get("/api/tasks"), api.get("/api/activities")]);
  ...
  const createTask = async (e) => {
    ...
    try {
      ...
      setErr(e.response?.data?.detail || e.response?.data?.error || "Failed to create task");
    } finally {
      setCreating(false);
    }
  };
  ...
  if (loading) return <div className="container"><h2>Employer Dashboard</h2><div className="muted">Loading...</div></div>;
      ...
      <div className="card">
        ...
        <form onSubmit={createTask}>
          <div style={{ marginBottom: "12px" }}>
            ...
            <input name="title" placeholder="Task title" required value={form.title}
              onChange={(e) => setForm(p => ({ ...p, title: e.target.value }))} />
          </div>
          <div style={{ marginBottom: "12px" }}>
            <label className="muted">Description</label>
            <textarea name="description" placeholder="Description (optional)" rows={3} value={form.description}
              onChange={(e) => setForm(p => ({ ...p, description: e.target.value }))} />
          </div>
          <div style={{ marginBottom: "12px" }}>
            <label className="muted">HR Email *</label>
            <input type="email" name="assigned_hr_email" placeholder="hr@example.com" required
              value={form.assigned_hr_email}
              onChange={(e) => setForm(p => ({ ...p, assigned_hr_email: e.target.value }))} />
          </div>
          <div style={{ marginBottom: "12px" }}>
